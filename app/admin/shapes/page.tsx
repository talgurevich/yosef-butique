...
export default function ShapesPage() {
  ...
  const fetchShapes = async () => {
    try {
      ...
      alert(`שגיאה בטעינת צורות: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async () => {
    try {
      ...
      alert(`שגיאה בהוספת צורה: ${error.message}`);
    }
  };
  ...
  const handleUpdate = async (id: string) => {
    try {
      ...
      alert(`שגיאה בעדכון צורה: ${error.message}`);
    }
  };
  ...
  const updateShape = (id: string, field: string, value: any) => {
    setShapes(
      shapes.map((shape) =>
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">ניהול צורות</h1>
          <p className="text-gray-600 mt-2">נהל את הצורות הזמינות למוצרים</p>
        </div>
        <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-gray-700 font-medium mb-2">
                שם הצורה <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={formData.name}
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                placeholder="תיאור הצורה (אופציונלי)"
              />
            </div>
            ...
            <div>
              <label className="flex items-center">
                ...
                />
                <span className="mr-3 text-gray-700">צורה פעילה</span>
              </label>
            </div>
          </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {editingId === shape.id ? (
                      <label className="flex items-center">
                        <input
                          type="checkbox"
                          checked={shape.is_active}
                          onChange={(e) =>
                            updateShape(shape.id, 'is_active', e.target.checked)
                          }
                          className="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                        />
