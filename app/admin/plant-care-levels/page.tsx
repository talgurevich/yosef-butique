...
export default function PlantCareLevelsPage() {
  ...
  const fetchPlantCareLevels = async () => {
    try {
      ...
      alert(`שגיאה בטעינת רמות טיפול: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async () => {
    try {
      if (!formData.name.trim()) {
        alert('נא למלא את שם רמת הטיפול');
        return;
      }
      ...
      alert(`שגיאה בהוספת רמת טיפול: ${error.message}`);
    }
  };
  ...
  const handleUpdate = async (id: string) => {
    try {
      ...
      alert(`שגיאה בעדכון רמת טיפול: ${error.message}`);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      alert(`שגיאה במחיקת רמת טיפול: ${error.message}`);
    }
  };

  const updatePlantCareLevel = (id: string, field: string, value: any) => {
    setPlantCareLevels(
      plantTypes.map((pt) =>
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">ניהול רמות טיפול</h1>
      ...
      {isAdding && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">רמת טיפול חדש</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-gray-700 font-medium mb-2">
                שם רמת הטיפול <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={formData.name}
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                placeholder="תיאור רמת הטיפול (אופציונלי)"
              />
            </div>
            ...
            <div>
              <label className="flex items-center">
                ...
                />
                <span className="mr-3 text-gray-700">רמת טיפול פעיל</span>
              </label>
            </div>
          </div>
            ...
            {plantTypes.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                  אין רמות טיפול עדיין. לחץ על "הוסף רמת טיפול חדש" כדי להתחיל.
                </td>
              </tr>
            ) : (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {editingId === plantType.id ? (
                      <input
                        ...
                        onChange={(e) => updatePlantCareLevel(plantType.id, 'name', e.target.value)}
                  ...
                  <td className="px-6 py-4">
                    {editingId === plantType.id ? (
                      <textarea
                        ...
                        onChange={(e) => updatePlantCareLevel(plantType.id, 'description', e.target.value)}
                        rows={2}
                        className="w-full px-3 py-1 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500"
                      />
                    ) : (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {editingId === plantType.id ? (
                      <input
                        ...
                        onChange={(e) =>
                          updatePlantCareLevel(plantType.id, 'sort_order', parseInt(e.target.value) || 0)
                        }
                    ...
                    {editingId === plantType.id ? (
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={(e) => updatePlantCareLevel(plantType.id, 'is_active', e.target.checked)}
                          className="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                        />
